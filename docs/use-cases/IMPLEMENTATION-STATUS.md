...
To validate a deployment, configure the existing `SIT_API_BASE`, `SIT_WEB_BASE`, test tenant/account and database settings, apply the engine registry schema through console startup, and run `npm run sit:use-cases`. Use the external console on port 8098 for selection, history and use-case links.
